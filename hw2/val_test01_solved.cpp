...
//
{
  ...
  // Old Code ::
  // for ( i = 2; i <= n; i++ )
  // i = n would refer to x[10], while array x is of size 10 i.e. from x[0] till x[9]
  for( i = 0; i < n; i++)
  {
    x[i] = x[i-1] + x[i-2];
    cout << "  " << i << "  " << x[i] << "\n";
  ...
  // delete [] x;
  // Allocation is done using malloc, so use free instead
  // As there is no gurantee for delete and malloc to be compatible
  free(x);
